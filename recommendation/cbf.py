...
def preprocess_metadata(df: pd.DataFrame, text_cols: list, list_cols: list):
    df = df.copy()
...
def recommend_items(user_id: str, user_hist: dict, meta_df: pd.DataFrame, tfidf_matrix, top_n: int = 5):
    ...
    for i in top_idx:
        ...
        score   = float(sims[i])
        price   = meta_df.iloc[i]['price']
        details = meta_df.iloc[i]['details']
        recs.append((asin, score, price, details))
    return recs
...
if __name__ == '__main__':
    ...
    for user_id in user_hist.keys():
        ...
        for asin, score, price, details in recs:
            all_rows.append({
                'user_id': user_id,
